app.controller('OralExamCtrl', ['$scope', '$http', function (s, h) {
    ...
    s.saveDiagnosis = function (diagnosisCheck, detail, remarks, personnelID) {
        if (s.qrData.qrCode == '' || s.qrData.qrCode == null) {
            ...
            if (s.bpHistoryList.length == 0) {
                ...
                h.post('../Dental/saveDentalDiagnosis', { qrCode: s.qrData.qrCode, checkedDiagnosis: listChecked, detail: detail, personnelID: personnelID, remarks: remarks }).then(function (d) {
                    if (d.data.status == "error") {
                        swal({
                            title: "ERROR",
    ...
    function getMedicineList() {
        h.post('../VitalSigns/getMedicineList?serviceID=SRV0006/SRV0010/SRV0021/SRV0008').then(function (d) {
            if (d.data.status == 'error') {
                swal({
                    title: "ERROR",
                    text: d.data.msg,
    ...
    s.savePrescription = function () {
        ...
        h.post('../Dental/savePrescription_Dental', { qrCode: s.qrData.qrCode, listRx: s.RxList }).then(function (d) {
            if (d.data.status == "error") {
                swal({
                    title: "ERROR",
